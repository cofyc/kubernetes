...
func TestInvalidatePredicateInFlight(t *testing.T) {
	...
	factory.RegisterFitPredicate(predicateName, func(pod *v1.Pod, _ algorithm.PredicateMetadata, _ *schedulercache.NodeInfo) (bool, []algorithm.PredicateFailureReason, error) {
		...
		version, ok := pod.Annotations["version"]
		if !ok || version != "pass" {
			fit = false
			failureReasons = append(failureReasons, &predicates.PredicateFailureError{PredicateName: predicateName, PredicateDesc: "invalid pod"})
		} else {
			fit = true
		}
	...
	defer func() {
		factory.RemoveFitPredicate(predicateName)
	}()
	savedOrdering := predicates.Ordering()
	predicates.SetPredicatesOrdering([]string{predicateName})
	defer func() {
		predicates.SetPredicatesOrdering(savedOrdering)
	}()
	...
	context := initTestSchedulerWithOptions(t, initTestMaster(t, "predicate-in-flight", nil), nil, false, policy, false, 0)
	defer cleanupTest(t, context)
	// Disable preemption for this test.
	context.schedulerConfig.DisablePreemption = true

	// Add a node.
	_, err := createNode(context.clientSet, "testnode", nil)
	if err != nil {
		t.Fatalf("Cannot create node: %v", err)
	}
	...
	testPod, err = context.clientSet.CoreV1().Pods(context.ns.Name).Create(testPod)
	if err != nil {
		t.Fatalf("Test Failed: error: %v, while creating pod", err)
	}
	...
	err = waitForPodUnschedulable(context.clientSet, testPod)
	if err != nil {
		t.Errorf("Test Failed: error, %v, while waiting for scheduled", err)
	}
	...
	err = waitForPodToSchedule(context.clientSet, testPod)
	if err != nil {
		t.Errorf("Test Failed: error, %v, while waiting for scheduled", err)
	}

	cleanupPods(context.clientSet, t, []*v1.Pod{testPod})
}
